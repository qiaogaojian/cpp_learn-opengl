...
int main()
{
    ...
    // 构建和编译 shader 程序
    //--------------------------------------------------------------------------------------
    char *vsPath = "/src/5.4 Normal Mapping/normal_mapping.vert";
    char *fsPath = "/src/5.4 Normal Mapping/normal_mapping.frag";
    ...
    // 第一个材质 漫反射贴图
    string texPath = shaderObject.concatString(getcwd(NULL, 0), "/res/texture/brickwall.jpg");
    unsigned int texture = loadTexture(texPath.c_str());

    // 第二个材质 镜面反射贴图
    texPath = shaderObject.concatString(getcwd(NULL, 0), "/res/texture/brickwall_normal.jpg");
